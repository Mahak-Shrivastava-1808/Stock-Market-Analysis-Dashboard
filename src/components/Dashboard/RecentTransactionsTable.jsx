...
const recentTransactions = [
  { id: "#1001", stock: "INFY", type: "Buy", quantity: 50, date: "2025-10-10" },
  { id: "#1002", stock: "TCS", type: "Sell", quantity: 20, date: "2025-10-11" },
  { id: "#1003", stock: "RELIANCE", type: "Buy", quantity: 10, date: "2025-10-12" },
];
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      const results = await Promise.all(
        recentTransactions.map((tx) => getStockPrice(tx.stock))
      );
        ...
        <tbody>
          {recentTransactions.map((tx, index) => (
            ...
            >
              ...
              <td className="p-2">
                {livePrices[tx.stock] !== undefined
                  ? `₹${livePrices[tx.stock].toLocaleString()}`
                  : "Loading..."}
              </td>
